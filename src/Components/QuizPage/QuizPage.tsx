  ...
  return (
    <div className="quizpage-main-container">
      ...
      <form onSubmit={handleSubmit(onSubmitt)}>
        <div className="form-group">
          {QuizList.map((ele) => {
            return ele.map((elee, indd) => {
              return veiwResult ? elee.category === id ? (
                    ...
                    return (
                      <div className="form-check" key={ind} >
                        ...
                        <label className="form-check-label" htmlFor={ele} >
                          {ele} {
                            elee.correct_answer === ele? <img src={Tick} style={{height: '15px' , width: '15px'}}/> : ''
                          }
                        </label>
                      </div>
                    );
       ...
       <br/><br/>
        <button className="btn btn-dark" type='button' style={{ display :  showButton? '' : 'none'}} onClick={()=>setVeiwResult(true)} >
          Veiw Answers
        </button>
      </form>
      <br/>
      <Link to='/'><button 
        className="btn btn-dark w-25" type="submit" style={{float: 'right'}}>
          Back
        </button> </Link>
